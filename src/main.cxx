...
//#include "itkGaussianRBFKernelTransform.h"
//#include "itkGaussianRBFSparseKernelTransform.h"
//#include "itkCompactlySupportedRBFSparseKernelTransform.h"
...
#include "vis.h"
#include "itkMultiplyByConstantImageFilter.h" // from ITKv4
...
//---------------------- for sorting ---------------------------------
bool isGreater(std::pair<double, int > elem1,
               std::pair<double, int > elem2);

//---------------------- statistics ---------------------------------
double computeMeanValue(std::vector<double> vals);
double computeStandardDeviation(std::vector<double> vals, double mean);

vtkSmartPointer< vtkPoints > computeMeanSparseShape(std::vector< std::string > localPointsFilenames,
                                                    int number_of_particles, int& medianShapeIndex, int do_procrustes_scaling = 0);
...
int main( int argc , char* argv[] )
{
    ...
    vtkSmartPointer< vtkPoints > meanSparseShape = computeMeanSparseShape(params.localPointsFilenames,
                                                                          params.number_of_particles, medianShapeIndex,
                                                                          params.do_procrustes_scaling);
#ifdef VIS_DEBUG
    if(params.display)
        visParticles(meanSparseShape,2,std::string("mean sparse shape"));
#endif
    ...
    for (unsigned int ii = 0 ; ii < params.number_of_particles; ii++)
    {
        ...
        for (unsigned int shapeNo_kk = 1; shapeNo_kk < params.localPointsFilenames.size(); shapeNo_kk++)
        {
            ...
            if((nx_jj*nx_kk + ny_jj*ny_kk + nz_jj*nz_kk) < 0)
            {
                isGood = false;
                break;
            }
    ...
    // now visualize
    if (params.display)
        visParticles(bad_ones, good_ones,1.5, std::string("bad particles in red and good ones are in green")); // bad in red and good in green
#endif
    ...
    //std::vector<int> particles_indices = chooseParticles(params, medianShapeIndex);
    ...
    for (unsigned int shapeNo = 0; shapeNo < params.distanceTransformFilenames.size(); shapeNo++)
    {
        ...
        vtkSmartPointer< vtkPoints > targetPts = readSparseShape((char*) params.localPointsFilenames[shapeNo].c_str(),
                                                                 params.number_of_particles);
...
#ifdef VIS_DEBUG
        if(params.display)
            visParticles(targetPts,meanSparseShape,2, std::string("target particles in red and mean sparse shape in green"));
#endif
...
#ifdef VIS_DEBUG
        if(params.display)
            visParticles(targetPts,mappedCorrespondences, 2, std::string("target particles in red and mapped correspondences in green"));
#endif
        ...
        for (int j = 0; j < params.number_of_attributes; j++)
        {
            ...
            std::string outfilename = params.out_path + '/'+ removeExtension(getFilename(params.attribute_labels[shapeNo+j])) + ".warped.nrrd";
        ...
        if(params.display)
        {
            ...
            visMeshWithParticles(denseShape, meanSparseShape, 1, std::string("dense shape in read and mean sparse shape in white"));


        }

        std::string outfilename = params.out_path + '/'+ removeExtension(getFilename(params.distanceTransformFilenames[shapeNo])) + ".warped.nrrd";
    ...
    writeSparseShape( (char*) meanShape_filename.c_str(), meanSparseShape);
    ...
    // Prateep
    //if (params.display)
        visMeshWithParticles(meanDenseShape,meanSparseShape, 1, std::string("mean dense shape in red and mean sparse shape in white"));
...
vtkSmartPointer< vtkPoints > computeMeanSparseShape(std::vector< std::string > localPointsFilenames,
                                                    int number_of_particles, int& medianShapeIndex, int do_procrustes_scaling)
{
    ...
    // procrustes.ConstructTransformMatrices(transforms,transformMatrices, do_procrustes_scaling); // not that tranforms scale is updated here if do_scaling ==1
                                           ...
                                           int number_of_particles)
{
    ...
    vtkSmartPointer<vtkPoints> particles =
            readSparseShape((char*) localPointsFilename.c_str(),  number_of_particles);

    vtkSmartPointer<vtkPolyData> particlesData =
            vtkSmartPointer<vtkPolyData>::New();
    particlesData->SetPoints(particles);

    vtkSmartPointer<vtkPoints> pts = convertToImageCoordinates(particlesData->GetPoints(),
    ...
    vtkSmartPointer<vtkPoints> pts2 = convertToPhysicalCoordinates(particlesData->GetPoints(),
                                                                   particlesData->GetPoints()->GetNumberOfPoints(), spacing, origin);
